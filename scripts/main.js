...
const app = Vue.createApp({
  ...
  methods: {
    ...
    contactFormValidation() {
      ...
      // phone validation
      if (phone.value.length > 0 && !/^[\+]?[(]?[0-9]{2}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/im.test(phone.value)) {
    ...
    sendContactFormMessage(form) {
      ...
      const sender = contactForm.querySelector('input[name="email"]');
      const name        = contactForm.querySelector('input[name="name"]');
      const phone       = contactForm.querySelector('input[name="phone"]');
      const message     = contactForm.querySelector('textarea');
      ...
      Email.send({
        Host : "smtp.gmail.com",
        Username : "gasteluivan007@gmail.com",
        Password : "nfaekbmrpqofwsox",
        To : 'gastelu.n.ivan@gmail.com',
        From : sender.value,
        Subject : "Mensaje de la web Ivan Gastelu",
        Body : "Nombre: " + name.value + "<br>" + "Teléfono: " + phone.value + "<br>" + "Mensaje: " + message.value
      }).then( message => {
        console.log(message);
        if (message === 'OK') {
